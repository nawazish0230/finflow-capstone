...
export class AuthService {
  ...
  async signToken(payload: { sub: string; email?: string }): Promise<TokenResult> {
  ...
  async loginWithEmailPassword(email: string, password: string): Promise<TokenResult> {
    const normalizedEmail = email.toLowerCase().trim();
    const user = await this.userModel.findOne({ email: normalizedEmail }).select('+password').exec();
    if (!user) {
