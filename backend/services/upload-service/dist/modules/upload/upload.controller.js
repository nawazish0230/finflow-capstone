...
let UploadController = class UploadController {
    ...
    async initiate(user, file, password) {
        if (!file?.buffer)
            throw new common_1.BadRequestException('No file uploaded. Send multipart form with key "file".');
        return this.uploadService.initiateUpload(user.sub, {
            buffer: file.buffer,
            originalname: file.originalname ?? 'statement.pdf',
        }, password);
    }
    async getStatus(user, documentId) {
...
], UploadController.prototype, "initiate", null);
__decorate([
